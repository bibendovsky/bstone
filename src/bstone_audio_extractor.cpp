...
void AudioExtractorImpl::write_non_digitized_audio_chunk(const AudioChunk& audio_chunk, bstone::Stream& stream)
{
	...
	bstone::globals::logger->write(("\tSample rate: " + std::to_string(dst_rate)).c_str());
	bstone::globals::logger->write(("\tSample count: " + std::to_string(sample_count)).c_str());
	bstone::globals::logger->write(("\tVolume factor: " + std::to_string(volume_factor)).c_str());
}
...
void AudioExtractorImpl::write_digitized_audio_chunk(const AudioChunk& audio_chunk, bstone::Stream& stream)
{
	...
	bstone::globals::logger->write(("\tSample count: " + std::to_string(data_size)).c_str());
	bstone::globals::logger->write(("\tVolume factor: " + std::to_string(volume_factor)).c_str());
}
...
void AudioExtractorImpl::extract_raw_audio_chunk(const std::string& dst_dir, const AudioChunk& audio_chunk)
{
	...
	globals::logger->write(file_name.c_str());
	...
	globals::logger->write(("\tSHA1: " + sha1_string).c_str());
}
