...
std::intptr_t File::read(void* buffer, std::intptr_t count) const
{
	...
	const auto posix_number_of_bytes_to_read = static_cast<::size_t>(std::min(count, (std::intptr_t)posix_file_max_int));
...
std::intptr_t File::write(const void* buffer, std::intptr_t count) const
{
	...
	const auto posix_file_descriptor = resource_.get();
	const auto posix_number_of_bytes_to_write = static_cast<::size_t>(std::min(count, (std::intptr_t)posix_file_max_int));
