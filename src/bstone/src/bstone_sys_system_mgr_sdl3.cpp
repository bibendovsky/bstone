...
class Sdl3SystemMgr final : public SystemMgr
{
public:
	Sdl3SystemMgr(Logger& logger);
	Sdl3SystemMgr(const Sdl3SystemMgr&) = delete;
	Sdl3SystemMgr& operator=(const Sdl3SystemMgr&) = delete;
	~Sdl3SystemMgr() override;
...
Sdl3SystemMgr::Sdl3SystemMgr(Logger& logger)
try
	:
	logger_{logger}
{
	...
	log_versions();
	sdl3_ensure_result(SDL_Init(0));
	sdl3_ensure_result(SDL_SetHint(SDL_HINT_VIDEO_SYNC_WINDOW_OPERATIONS, "1"));

	audio_mgr_ = make_sdl3_audio_mgr(logger_);
	event_mgr_ = make_sdl3_event_mgr(logger_);
	video_mgr_ = make_sdl3_video_mgr(logger_);

	logger_.log_information(">>> SDL system manager started up.");
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

Sdl3SystemMgr::~Sdl3SystemMgr()
{
...
void* Sdl3SystemMgr::operator new(std::size_t size)
try {
	return get_memory_resource().allocate(size);
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

void Sdl3SystemMgr::operator delete(void* ptr)
{
	get_memory_resource().deallocate(ptr);
}

Logger& Sdl3SystemMgr::do_get_logger()
{
	return logger_;
}

AudioMgr& Sdl3SystemMgr::do_get_audio_mgr()
{
	return *audio_mgr_;
}

EventMgr& Sdl3SystemMgr::do_get_event_mgr()
{
	return *event_mgr_;
}

VideoMgr& Sdl3SystemMgr::do_get_video_mgr()
{
	return *video_mgr_;
}

MemoryResource& Sdl3SystemMgr::get_memory_resource()
{
	static SinglePoolResource<Sdl3SystemMgr> memory_pool{};

	return memory_pool;
}

void Sdl3SystemMgr::log_version(int sdl_version, std::string_view version_name)
{
	char major_chars[3];
...
try {
	return std::make_unique<Sdl3SystemMgr>(logger);
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED
