...
using Sdl3GlContextUPtr = std::unique_ptr<std::remove_pointer_t<SDL_GLContext>, Sdl3GlContextDeleter>;

// ==========================================================================

class Sdl3GlContext final : public GlContext
{
public:
	Sdl3GlContext(Logger& logger, SDL_Window& sdl_window);
	Sdl3GlContext(const Sdl3GlContext&) = delete;
	Sdl3GlContext& operator=(const Sdl3GlContext&) = delete;
...
private:
	Logger& logger_;
	Sdl3GlContextUPtr sdl_context_{};
...
Sdl3GlContext::Sdl3GlContext(Logger& logger, SDL_Window& sdl_window)
try
	:
	logger_{logger}
{
	...
	detail::sdl3_log_eol(message);
	message += "  Window ptr: ";
	detail::sdl3_log_xint_hex(reinterpret_cast<std::uintptr_t>(&sdl_window), message);
	logger_.log_information(message.c_str());

	auto sdl_context = Sdl3GlContextUPtr{sdl3_ensure_result(SDL_GL_CreateContext(&sdl_window))};
	...
	message.clear();
	message += "Ptr: ";
	detail::sdl3_log_xint_hex(reinterpret_cast<std::uintptr_t>(sdl_context_.get()), message);
	detail::sdl3_log_eol(message);
	message += "Effective attributes:";
	detail::sdl3_log_eol(message);
	detail::sdl3_log_gl_attributes(attributes_, message);
...
void* Sdl3GlContext::operator new(std::size_t size)
try {
	return get_memory_resource().allocate(size);
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

void Sdl3GlContext::operator delete(void* ptr)
{
	get_memory_resource().deallocate(ptr);
}

const GlContextAttributes& Sdl3GlContext::do_get_attributes() const noexcept
{
	return attributes_;
}

MemoryResource& Sdl3GlContext::get_memory_resource()
{
	struct Initializer
	{
		Initializer(GenericPoolResource& generic_memory_pool)
		{
			generic_memory_pool.reserve(
				static_cast<std::intptr_t>(sizeof(Sdl3GlContext)),
				sys_max_gl_contexts,
...
GlContextProfile Sdl3GlContext::map_profile(int sdl_context_profile)
try {
...
int Sdl3GlContext::get_attrib(SDL_GLAttr gl_attrib)
try {
	auto gl_value = 0;
	sdl3_ensure_result(SDL_GL_GetAttribute(gl_attrib, &gl_value));
	return gl_value;
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

bool Sdl3GlContext::get_attrib_bool(SDL_GLAttr gl_attrib)
try {
...
GlContextUPtr make_sdl3_gl_context(Logger& logger, SDL_Window& sdl_window)
{
	return std::make_unique<Sdl3GlContext>(logger, sdl_window);
}
