...
#include "bstone_sys_gl_symbol_resolver_sdl3.h"

namespace bstone {
namespace sys {

class Sdl3GlCurrentContext final : public GlCurrentContext
{
public:
	Sdl3GlCurrentContext(Logger& logger);
	~Sdl3GlCurrentContext() override;
...
private:
	Logger& logger_;
	Sdl3GlSymbolResolver gl_symbol_resolver_{};

	static SinglePoolResource<Sdl3GlCurrentContext> memoryResource_;
...
SinglePoolResource<Sdl3GlCurrentContext> Sdl3GlCurrentContext::memoryResource_{};

Sdl3GlCurrentContext::Sdl3GlCurrentContext(Logger& logger)
	:
	logger_{logger}
{
...
Sdl3GlCurrentContext::~Sdl3GlCurrentContext()
{
	logger_.log_information("Shut down OpenGL current context.");
}

void* Sdl3GlCurrentContext::operator new(std::size_t size)
{
	return memoryResource_.allocate(static_cast<std::intptr_t>(size));
}

void Sdl3GlCurrentContext::operator delete(void* ptr) noexcept
{
	memoryResource_.deallocate(ptr);
}

bool Sdl3GlCurrentContext::do_has_extension(const char* extension_name) const noexcept
{
	return SDL_GL_ExtensionSupported(extension_name);
}

SwapIntervalType Sdl3GlCurrentContext::do_get_swap_interval() const noexcept
{
...
void Sdl3GlCurrentContext::do_set_swap_interval(SwapIntervalType swap_interval_type)
{
	const auto sdl_swap_interval = map(swap_interval_type);
	sdl3_ensure_result(SDL_GL_SetSwapInterval(sdl_swap_interval));
}

const GlSymbolResolver& Sdl3GlCurrentContext::do_get_symbol_resolver() const noexcept
{
	return gl_symbol_resolver_;
}

int Sdl3GlCurrentContext::map(SwapIntervalType swap_interval_type)
{
...
GlCurrentContextUPtr make_sdl3_gl_current_context(Logger& logger)
{
	return std::make_unique<Sdl3GlCurrentContext>(logger);
}
