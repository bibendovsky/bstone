...
try {
	...
	const VkImageCreateInfo vk_image_create_info{
		...
		.format                = vk_format,
		.extent                = VkExtent3D{.width  = vk_width, .height = vk_height, .depth  = 1},
	...
	std::uint32_t vk_mip_level) const
{
	const VkBufferImageCopy vk_buffer_image_copy{
		...
		.imageExtent       = VkExtent3D{.width  = vk_width, .height = vk_height, .depth  = 1}};
	...
	VkBuffer vk_buffer) const
{
	const VkBufferImageCopy vk_buffer_image_copy{
		...
		.imageSubresource  = VkImageSubresourceLayers{
			...
			.layerCount     = 1},
		.imageOffset       = VkOffset3D{.x = 0, .y = 0, .z = 0},
		.imageExtent       = VkExtent3D{.width  = vk_image_width, .height = vk_image_height, .depth  = 1}};
	...
	VkFilter vk_filter) const
{
	const VkImageBlit vk_image_blit{
		...
		.srcOffsets     = {
			...
			VkOffset3D{.x = src_vk_width, .y = src_vk_height, .z = 1}},
		...
		.dstOffsets     = {
			VkOffset3D{.x = 0, .y = 0, .z = 0},
			VkOffset3D{.x = dst_vk_width, .y = dst_vk_height, .z = 1}}};
	vkCmdBlitImage(
		/* commandBuffer */  vk_command_buffer,
...
void VkR3rContext::draw_state_update_scissor()
{
	draw_state.scissor = VkRect2D{
		...
		.extent = VkExtent2D{.width =  vk_offscreen_width, .height = vk_offscreen_height}};
}
