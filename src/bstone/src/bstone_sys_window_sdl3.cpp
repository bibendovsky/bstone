...
#include "bstone_sys_gl_context_sdl3.h"
#include "bstone_sys_renderer_sdl3.h"

namespace bstone {
namespace sys {

Sdl3WindowInternal::Sdl3WindowInternal() = default;

Sdl3WindowInternal::~Sdl3WindowInternal() = default;

void* Sdl3WindowInternal::get_native_handle() const noexcept
{
	return do_get_native_handle();
}

void* Sdl3WindowInternal::get_sdl_window() const
{
...
using Sdl3WindowUPtr = std::unique_ptr<SDL_Window, Sdl3WindowDeleter>;

// ==========================================================================

class Sdl3Window final : public Sdl3WindowInternal
{
public:
	Sdl3Window(
		Logger& logger,
	...
	rounded_corner_mgr_{rounded_corner_mgr}
{
	...
	sdl_window_.reset(sdl3_ensure_result(SDL_CreateWindow(
		param.title,
		param.width,
		param.height,
		sdl_flags)));
	sdl3_ensure_result(SDL_SetWindowPosition(sdl_window_.get(), sdl_x, sdl_y));
...
Sdl3Window::~Sdl3Window()
{
	...
	detail::sdl3_log_xint(sdl_window_id_, message);
	message += "; ptr:";
	detail::sdl3_log_xint_hex(reinterpret_cast<std::uintptr_t>(sdl_window_.get()), message);
	message += ").";

	logger_.log_information(message.c_str());
}

void* Sdl3Window::operator new(std::size_t size)
try {
	return get_memory_resource().allocate(size);
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

void Sdl3Window::operator delete(void* ptr)
{
	get_memory_resource().deallocate(ptr);
}

void* Sdl3Window::do_get_native_handle() const noexcept
{
#ifdef _WIN32
...
void* Sdl3Window::do_get_sdl_window() const
{
	return sdl_window_.get();
}

const char* Sdl3Window::do_get_title()
try {
	return sdl3_ensure_result(SDL_GetWindowTitle(sdl_window_.get()));
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

void Sdl3Window::do_set_title(const char* title)
try {
	SDL_SetWindowTitle(sdl_window_.get(), title);
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

WindowPosition Sdl3Window::do_get_position()
try {
	auto x = 0;
	auto y = 0;
...
void Sdl3Window::do_set_position(WindowPosition position)
try {
...
WindowSize Sdl3Window::do_get_size()
try {
	auto width = 0;
	auto height = 0;
...
void Sdl3Window::do_set_size(WindowSize size)
try {
	sdl3_ensure_result(SDL_SetWindowSize(sdl_window_.get(), size.width, size.height));
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

DisplayMode Sdl3Window::do_get_display_mode()
try {
	...
	const SDL_DisplayMode* const sdl_display_mode = sdl3_ensure_result(SDL_GetCurrentDisplayMode(SDL_GetPrimaryDisplay()));
	int sdl_w;
	int sdl_h;
	sdl3_ensure_result(SDL_GetWindowSizeInPixels(sdl_window, &sdl_w, &sdl_h));
	return DisplayMode{
		.width = sdl_w,
		.height = sdl_h,
...
void Sdl3Window::do_set_display_mode(const DisplayMode& display_mode)
try {
	SDL_DisplayMode sdl_display_mode{};
	sdl3_ensure_result(SDL_GetClosestFullscreenDisplayMode(
		...
		false,
		&sdl_display_mode));
	sdl3_ensure_result(SDL_SetWindowFullscreenMode(sdl_window_.get(), &sdl_display_mode));
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

void Sdl3Window::do_show(bool is_visible)
try {
	const auto sdl_func = is_visible ? SDL_ShowWindow : SDL_HideWindow;
	sdl_func(sdl_window_.get());
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

void Sdl3Window::do_set_rounded_corner_type(WindowRoundedCornerType value)
try {
	rounded_corner_mgr_.set_round_corner_type(*this, value);
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

WindowFullscreenType Sdl3Window::do_get_fullscreen_mode()
{
...
void Sdl3Window::do_set_fullscreen_mode(WindowFullscreenType fullscreen_mode)
try {
	...
	{
		sdl3_ensure_result(SDL_SetWindowFullscreenMode(sdl_window_.get(), nullptr));
	}
	sdl3_ensure_result(SDL_SetWindowFullscreen(sdl_window_.get(), is_fullscreen));
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

GlContextUPtr Sdl3Window::do_make_gl_context()
try {
	return make_sdl3_gl_context(logger_, *sdl_window_);
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

WindowSize Sdl3Window::do_gl_get_drawable_size()
try {
	int sdl_width;
	int sdl_height;
	sdl3_ensure_result(SDL_GetWindowSizeInPixels(sdl_window_.get(), &sdl_width, &sdl_height));
	return WindowSize{sdl_width, sdl_height};
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

void Sdl3Window::do_gl_swap_buffers()
try {
	SDL_GL_SwapWindow(sdl_window_.get());
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

RendererUPtr Sdl3Window::do_make_renderer(const RendererInitParam& param)
try {
	return make_sdl3_renderer(logger_, *sdl_window_, param);
} BSTONE_END_FUNC_CATCH_ALL_THROW_NESTED

MemoryResource& Sdl3Window::get_memory_resource()
{
	static SinglePoolResource<Sdl3Window> memory_pool{};

	return memory_pool;
}

void Sdl3Window::log_offset(WindowOffset offset, std::string& message)
{
	switch (offset.get_type())
	{
		...
		default:
			detail::sdl3_log_xint(offset.get(), message);
			break;
	}
}

void Sdl3Window::log_rect(const WindowInitParam& param, std::string& message)
{
	// x
	//
	message += "(x:";
	...
	// width
	//
	detail::sdl3_log_xint(param.width, message);
	message += "; h:";

	// height
	//
	detail::sdl3_log_xint(param.height, message);
	message += ')';
}

void Sdl3Window::log_flag(const char* flag_name, std::string& message)
{
...
void Sdl3Window::log_flags(const WindowInitParam& param, std::string& message)
{
...
void Sdl3Window::log_rounded_corner_type(const WindowInitParam& param, std::string& message)
{
...
void Sdl3Window::log_gl_attributes(const GlContextAttributes* gl_attributes, std::string& message)
{
	message += "  ";
	...
	{
		detail::sdl3_log_gl_attributes(*gl_attributes, message);
	}
	else
	{
		const auto default_gl_attribs = make_default_gl_attributes();
		detail::sdl3_log_gl_attributes(default_gl_attribs, message);
	}
}

void Sdl3Window::log_input(const WindowInitParam& param, std::string& message)
{
...
void Sdl3Window::log_output(std::string& message)
{
	//
	...
	message += "  Rect: (x:";
	detail::sdl3_log_xint(x, message);
	message += "; y:";
	detail::sdl3_log_xint(y, message);
	...
	message += "; w:";
	detail::sdl3_log_xint(width, message);
	message += "; h:";
	detail::sdl3_log_xint(height, message);
	message += ')';
	detail::sdl3_log_eol(message);
}

int Sdl3Window::map_offset(WindowOffset offset)
{
	switch (offset.get_type())
	{
...
Uint32 Sdl3Window::map_flags(const WindowInitParam& param) noexcept
{
	Uint32 sdl_flags = 0;
...
int Sdl3Window::map_gl_context_profile(GlContextProfile context_profile)
try {
...
GlContextAttributes Sdl3Window::make_default_gl_attributes() noexcept
{
...
void Sdl3Window::set_gl_attributes(const GlContextAttributes& gl_attribs)
{
	const auto sdl_profile = map_gl_context_profile(gl_attribs.profile);
	const int sdl_accelerated = gl_attribs.is_accelerated;

	SDL_GL_ResetAttributes();
	sdl3_ensure_result(SDL_GL_SetAttribute(SDL_GL_ACCELERATED_VISUAL, sdl_accelerated));
	sdl3_ensure_result(SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, sdl_profile));
	sdl3_ensure_result(SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, gl_attribs.major_version));
	sdl3_ensure_result(SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, gl_attribs.minor_version));
	sdl3_ensure_result(SDL_GL_SetAttribute(SDL_GL_MULTISAMPLEBUFFERS, gl_attribs.multisample_buffer_count));
	sdl3_ensure_result(SDL_GL_SetAttribute(SDL_GL_MULTISAMPLESAMPLES, gl_attribs.multisample_sample_count));
	sdl3_ensure_result(SDL_GL_SetAttribute(SDL_GL_RED_SIZE, gl_attribs.red_bit_count));
	sdl3_ensure_result(SDL_GL_SetAttribute(SDL_GL_GREEN_SIZE, gl_attribs.green_bit_count));
	sdl3_ensure_result(SDL_GL_SetAttribute(SDL_GL_BLUE_SIZE, gl_attribs.blue_bit_count));
	sdl3_ensure_result(SDL_GL_SetAttribute(SDL_GL_ALPHA_SIZE, gl_attribs.alpha_bit_count));
	sdl3_ensure_result(SDL_GL_SetAttribute(SDL_GL_DEPTH_SIZE, gl_attribs.depth_bit_count));
}
...
WindowUPtr make_sdl3_window(
	Logger& logger,
	WindowRoundedCornerMgr& rounded_corner_mgr,
	const WindowInitParam& param)
{
	return std::make_unique<Sdl3Window>(logger, rounded_corner_mgr, param);
}
