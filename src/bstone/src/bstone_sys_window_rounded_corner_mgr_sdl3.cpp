...
// Window rounded corner manager (SDL3).
...
class Sdl3WindowRoundedCornerMgr final : public WindowRoundedCornerMgr
{
public:
	Sdl3WindowRoundedCornerMgr();
	~Sdl3WindowRoundedCornerMgr() override;
...
private:
	...
	using Pool = SinglePoolResource<Sdl3WindowRoundedCornerMgr>;
...
Sdl3WindowRoundedCornerMgr::Pool Sdl3WindowRoundedCornerMgr::pool_{};

// --------------------------------------------------------------------------

Sdl3WindowRoundedCornerMgr::Sdl3WindowRoundedCornerMgr()
{
...
Sdl3WindowRoundedCornerMgr::~Sdl3WindowRoundedCornerMgr() = default;

void* Sdl3WindowRoundedCornerMgr::operator new(std::size_t size)
{
	return pool_.allocate(size);
}

void Sdl3WindowRoundedCornerMgr::operator delete(void* ptr) noexcept
{
	pool_.deallocate(ptr);
}

void Sdl3WindowRoundedCornerMgr::do_set_round_corner_type(
	Window& window,
	WindowRoundedCornerType rounded_corner_type)
{
	...
	const auto native_handle = static_cast<Sdl3WindowInternal&>(window).get_native_handle();
...
{
	return std::make_unique<Sdl3WindowRoundedCornerMgr>();
}
