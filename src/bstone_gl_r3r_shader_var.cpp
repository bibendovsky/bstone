...
try
{
	if (type_id != type_id_)
	{
		BSTONE_STATIC_THROW("Mismatch type.");
	}

	if (!value_data)
	{
		BSTONE_STATIC_THROW("Null value data.");
	}
	...
	{
		BSTONE_STATIC_THROW("Value size mismatch.");
	}
	...
	switch (type_)
	{
		...
		default: BSTONE_STATIC_THROW("Changing vertex attribute not supported.");
	}
